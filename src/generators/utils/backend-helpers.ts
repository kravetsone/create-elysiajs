 ...
 */
export function getDrizzleDriversMap(database: string, runtime: "Bun" | "Node.js") {
...
export async function generateDatabaseFiles(
  ...
  const dbDir = isMonorepo ? `${projectDir}/apps/backend/src/libs` : `${projectDir}/src/db`;
  ...
  if (preferences.orm === "Drizzle") {
    ...
    if (preferences.database === "SQLite") {
      const dbPath = isMonorepo ? `${projectDir}/apps/backend/sqlite.db` : `${projectDir}/sqlite.db`;
